...
2026-09-09 的检查确认旧版最终 PNG 一致不能证明中途可见。连续自发 BridgeTalk 消息在大图中仍使界面检查超时；当前实现由 Mac AppleScript 或 Windows COM 启动器分批调用，在 Illustrator 进程外等待 500 ms。每批最多 8 项操作，文本保留逐字符字体与基线属性。创建分组后优先使用本批的对象引用，避免 Illustrator 集合缓存尚未更新时 `getByName()` 找不到新分组。
