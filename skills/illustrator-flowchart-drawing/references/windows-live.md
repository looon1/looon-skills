...
含公式的任务先按 [公式排版与高保真验收](formulas-fidelity.md) 完成排版、字体检查与对象适配。公式整组属于首批静态结构，即使转为轮廓也不能逐字符播放。当前脚本未内置数学排版或复合路径适配；不能将公式塞入 `labels`、丢弃孔洞或使用图片来绕过限制。此要求同时适用于 Mac 和 Windows，不代表新增公式流程已经实机验证。
