...
$runtimePath = Join-Path $PSScriptRoot 'illustrator_flowchart_cached_runtime.jsx'
...
$prepareOutput = & $PythonExecutable @prepareArguments
...
function Invoke-CachedRuntime([object]$illustrator, [object]$configuration) {
    ...
    $bootstrap = "var ILLUSTRATOR_FLOWCHART_CACHED_CONFIG = $configJson; `$`.evalFile(new File($runtimeJson));"
...
try {
    ...
    foreach ($batchState in $state.batches) {
        ...
        $payload = [ordered]@{ viewBox = $cache.view_box; atoms = $atoms; clip = $batchState.clip }
    ...
    Write-Output "ILLUSTRATOR_FLOWCHART_COMPLETE|cache=$cachePath|ai=$OutputAi|png=$OutputPng|batches=$completedCount/$($state.batches.Count)|mode=$mode|illustrator_window_untouched=true"
} finally {
