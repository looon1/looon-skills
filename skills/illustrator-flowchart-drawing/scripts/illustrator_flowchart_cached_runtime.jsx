(function () {
  if (typeof ILLUSTRATOR_FLOWCHART_CACHED_CONFIG === "undefined") {
    return "ERROR|Missing ILLUSTRATOR_FLOWCHART_CACHED_CONFIG";
  }

  var config = ILLUSTRATOR_FLOWCHART_CACHED_CONFIG;
  ...
  function gradientColor(definition) {
    var gradientName = "ILLUSTRATOR_FLOWCHART_GRADIENT_" + definition.id;
    var gradient = null;
    for (var gradientIndex = 0; gradientIndex < app.activeDocument.gradients.length; gradientIndex += 1) {
      try {
        var candidate = app.activeDocument.gradients[gradientIndex];
        if (candidate.name === gradientName) {
          gradient = candidate;
          break;
        }
      } catch (ignoredGradient) {}
    }
    if (gradient === null) {
      gradient = app.activeDocument.gradients.add();
      try { gradient.name = gradientName; } catch (ignoredGradientName) {}
    }
    gradient.type = GradientType.LINEAR;
    while (gradient.gradientStops.length < definition.stops.length) gradient.gradientStops.add();
    while (gradient.gradientStops.length > definition.stops.length && gradient.gradientStops.length > 2) {
      gradient.gradientStops[gradient.gradientStops.length - 1].remove();
    }
    for (var stopIndex = 0; stopIndex < definition.stops.length; stopIndex += 1) {
      var sourceStop = definition.stops[stopIndex];
      var destinationStop = gradient.gradientStops[stopIndex];
      destinationStop.rampPoint = sourceStop.rampPoint;
      destinationStop.midPoint = 50;
      destinationStop.color = rgbColor(sourceStop.color);
      destinationStop.opacity = sourceStop.opacity;
    }
    var color = new GradientColor();
    color.gradient = gradient;
    color.angle = definition.angleDegrees;
    return color;
  }
  ...
  function writeAppearance(style, destination, scale) {
    ...
    if (style.filled) {
      destination.fillColor = style.fillGradient ? gradientColor(style.fillGradient) : rgbColor(style.fillColor);
    }
    ...
    if (style.stroked) {
      ...
      if (style.strokeDashes && style.strokeDashes.length) {
        var dashScale = style.nonScalingStroke ? 1 : scale;
        var mappedDashes = [];
        for (var dashIndex = 0; dashIndex < style.strokeDashes.length; dashIndex += 1) {
          mappedDashes.push(style.strokeDashes[dashIndex] * dashScale);
        }
        destination.strokeDashes = mappedDashes;
        destination.strokeDashOffset = style.strokeDashOffset * dashScale;
      } else {
  ...
  function mergedClipGeometry(subpaths) {
    if (!subpaths || !subpaths.length) throw new Error("Clip has no drawable subpaths");
    function clonePoint(point) {
      return {
        a: [point.a[0], point.a[1]],
        l: [point.l[0], point.l[1]],
        r: [point.r[0], point.r[1]],
        t: point.t
      };
    }
    var merged = [];
    var base = subpaths[0];
    for (var baseIndex = 0; baseIndex < base.points.length; baseIndex += 1) merged.push(clonePoint(base.points[baseIndex]));
    merged.push(clonePoint(base.points[0]));
    for (var subpathIndex = 1; subpathIndex < subpaths.length; subpathIndex += 1) {
      var current = subpaths[subpathIndex];
      if (!current.points || current.points.length < 2) continue;
      merged.push(clonePoint(current.points[0]));
      for (var pointIndex = 1; pointIndex < current.points.length; pointIndex += 1) {
        merged.push(clonePoint(current.points[pointIndex]));
      }
      merged.push(clonePoint(current.points[0]));
      merged.push(clonePoint(base.points[0]));
    }
    return {closed: true, points: merged};
  }

  function ensureBatchClip(batchGroup, clip, viewBox, scale, originLeft, originTop) {
    if (!clip || (!clip.rect && !clip.subpaths)) return null;
    var clipName = "ILLUSTRATOR_FLOWCHART_CACHE_CLIP_" + clip.id;
    var clipArtwork = findNamedArtwork(batchGroup, clipName);
    if (clipArtwork !== null && clipArtwork.typename === "CompoundPathItem") {
      try { clipArtwork.remove(); } catch (ignoredOldCompoundClip) {}
      clipArtwork = null;
    }
    if (clipArtwork === null && clip.rect) {
      var rect = clip.rect;
      var topLeft = mapPoint([rect[0], rect[1]], viewBox, scale, originLeft, originTop);
      clipArtwork = batchGroup.pathItems.rectangle(topLeft[1], topLeft[0], rect[2] * scale, rect[3] * scale);
      clipArtwork.name = clipName;
    }
    if (clipArtwork === null && clip.subpaths && clip.subpaths.length === 1) {
      clipArtwork = batchGroup.pathItems.add();
      writeGeometry(clip.subpaths[0], clipArtwork, viewBox, scale, originLeft, originTop);
      clipArtwork.name = clipName;
    }
    if (clipArtwork === null && clip.subpaths && clip.subpaths.length > 1) {
      // Illustrator scripting accepts only a top-level PathItem when toggling
      // GroupItem.clipped. Join disconnected contours with retraced zero-area
      // bridges and use even-odd fill, which is geometrically equivalent to a
      // compound clipping path while remaining one native PathItem.
      clipArtwork = batchGroup.pathItems.add();
      writeGeometry(mergedClipGeometry(clip.subpaths), clipArtwork, viewBox, scale, originLeft, originTop);
      clipArtwork.name = clipName;
    }
    clipArtwork.stroked = false;
    clipArtwork.filled = false;
    clipArtwork.evenodd = !!clip.evenOdd;
    clipArtwork.clipping = true;
    clipArtwork.zOrder(ZOrderMethod.BRINGTOFRONT);
    batchGroup.clipped = true;
    return clipArtwork;
  }
  ...
  function drawBatch() {
    ...
    for (var atomIndex = 0; atomIndex < payload.atoms.length; atomIndex += 1) {
      ...
      try {
        ...
        createdCount += 1;
        if ((atomIndex + 1) % 8 === 0 || atomIndex + 1 === payload.atoms.length) app.redraw();
    ...
    try {
      ensureBatchClip(batchGroup, payload.clip, viewBox, scale, origin[0], origin[1]);
    } catch (clipError) {
      return [
        "PARTIAL",
        "created=" + createdCount,
        "skipped=" + skippedCount,
        "total=" + payload.atoms.length,
        "failed=CLIP_" + (payload.clip ? payload.clip.id : "unknown"),
